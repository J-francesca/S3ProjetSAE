...
<div class="vote-section">
    ...
    <?php if ($voteStatus === 'terminer' || $voteStatus === null || !$voteExists): ?>
		...
		<?php elseif ($voteInfo['canRequestVote']): 
			if (!ResultatVoteFormel::existeDemandeVoteFormel($idProp, $idMembre)): ?>
				<a href="routeur.php?controleur=proposition&action=demanderVoteFormel&id=<?php echo $idProp; ?>&idMembre=<?php echo $idMembre; ?>&idGroupe=<?php echo $idGroupe; ?>"
				class="btn btn-warning">Demander un vote formel</a>
			<?php else: ?>
				<?php if (!ResultatVoteFormel::verifierNbVoteFormel($idProp,$idGroupe)): ?>
					
					<p>Votre demande de vote formel a été enregistrée. Attendez 
					la réponse des autres. </p>
				<?php else: ?>
					<?php 
						Vote::createVote($idProp, 24, 'majorite'); 
						header("Location:routeur.php?controleur=proposition&action=lireProposition&id=$idProp&idMembre=$idMembre&idGroupe=$idGroupe");
						exit();
					?>
				<?php endif; ?>
			<?php endif; ?>
		...
		<?php elseif ($voteExists && $voteStatus === 'en cours'): ?>
			...
			<?php else: ?>
				<?php
				$lesVotes = ResultatVote::getResultatByIdVote($idVote);
				$compteurTotalVotes = 0;
				$compteurResultatPositif = 0;
				
				foreach ($lesVotes as $vote) {
					$compteurTotalVotes++; 
					if ($vote->getResultat() == 1) { 
						$compteurResultatPositif++; 
					}
				}

				$compteurResultatNegatif = $compteurTotalVotes - $compteurResultatPositif;

				if ($compteurResultatPositif === 1) {
					$textePositif = "1 personne a voté positivement";
				} else {
					$textePositif = "$compteurResultatPositif personnes ont voté positivement";
				}

				if ($compteurResultatNegatif === 1) {
					$texteNegatif = "1 personne a voté négativement";
				} else {
					$texteNegatif = "$compteurResultatNegatif personnes ont voté négativement";
				}
				?>

				<p>Concernant cette proposition, <?php echo $textePositif; ?> et 
				<?php echo $texteNegatif; ?></p>
			<?php endif; ?>
		<?php endif; ?>
